										   ...
										   int nOfColumns)
{
  ...
  *cost = 0;
  ...
  if (nOfRows <= nOfColumns)
  {
	...
	/* Steps 1 and 2a */
	//			distMatrixIn[i + nRows * j] = DistMatrix[i][j];
	// cout << "nOfColumns = " << nOfColumns << endl;
	// for (row = 0; row < nOfRows; row++)
	// {
	// 	for (col = 0; col < nOfColumns; col++)
	// 	{
	// 		cout << distMatrix[row + nOfRows * col] << " ";
	// 	}
	// 	cout<<endl;
	// }
	for (row = 0; row < nOfRows; row++)
	  for (col = 0; col < nOfColumns; col++)
	  {
		if (fabs(distMatrix[row + nOfRows * col]) < DBL_EPSILON)
		  if (!coveredColumns[col])  // false in
		  {
			starMatrix[row + nOfRows * col] = true;
			coveredColumns[col] = true;
			// cout << "coveredColumns[" << col << "]=" << coveredColumns[col] << endl;
			break;
		  }
	  }
  }
								...
								int minDim)
{
  ...
  for (col = 0; col < nOfColumns; col++)
	if (coveredColumns[col])
	  nOfCoveredColumns++;
  // cout << "nOfCoveredColumns=" << nOfCoveredColumns << endl;
  // cout << "minDim=" << minDim << endl;
